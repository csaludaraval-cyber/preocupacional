    ...
    <div className="bg-white border shadow-2xl rounded-xl overflow-hidden">
      <Table>
        <TableHeader className="bg-slate-900">
          <TableRow>
            <TableHead className="py-4 px-6 text-[10px] uppercase font-black text-white tracking-widest">ID Orden / Fecha</TableHead>
          ...
          {data.length > 0 ? data.map((quote) => {
            ...
            return (
              <TableRow key={quote.id} className="text-xs hover:bg-slate-50 transition-colors border-slate-100">
                ...
                <TableCell>
                  <div className="flex flex-col">
                      <span className="font-black text-slate-700 uppercase tracking-tighter">{quote.empresaData?.razonSocial}</span>
                      <span className="text-[9px] text-slate-400 font-bold tracking-widest uppercase">
                        {quote.empresaData?.rut} | <span className={isFrecuente ? "text-blue-600" : "text-slate-400"}>{isFrecuente ? 'FRECUENTE' : 'NORMAL'}</span>
                      </span>
                  </div>
                </TableCell>
                <TableCell>
            ...
            <TableRow><TableCell colSpan={4} className="text-center py-20 text-slate-400 text-[10px] uppercase font-bold tracking-widest">No hay registros en esta categoría</TableCell></TableRow>
          )}
        </TableBody>
      </Table>
    </div>
    ...
    <div className="container mx-auto p-4 max-w-7xl font-sans pb-20">
      <div className="flex justify-between items-end mb-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button onClick={() => refetchQuotes()} variant="outline" size="sm" className="h-10 w-10 border-slate-200"><RefreshCw className="h-4 w-4 text-slate-400" /></Button>
        ...
        <DialogContent className="max-w-5xl max-h-[95vh] overflow-y-auto p-0 border-none shadow-2xl bg-slate-100">
          ...
          {quoteToManage && (
            <div className="flex flex-col gap-6 pb-20">
              ...
              <div className="bg-white shadow-sm mx-auto w-full max-w-4xl rounded-lg overflow-hidden"><DetalleCotizacion quote={quoteToManage} /></div>
