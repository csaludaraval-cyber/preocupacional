...
export default function AdminCotizaciones() {
  ...
  const handleTestLioren = async () => {
    ...
    try {
      ...
      if (result.success) {
        // CORRECCIÓN: Uso de optional chaining (?.) para evitar el error de undefined
        toast({ title: "Conexión OK", description: result.data?.rs || "Credenciales verificadas" });
      } else {
  ...
  const handleInvoiceNow = async (id: string) => {
    ...
    try {
      ...
      if (result.success) {
        toast({ title: "Éxito", description: `Factura ${result.folio} generada correctamente.` });
        await refetchQuotes();
  ...
  const handleSendEmail = async (quote: any) => {
    ...
    try {
      ...
      toast({ variant: 'destructive', title: "Error Envío", description: err.message }); 
    } finally { 
      setIsProcessing(null); 
    }
  };
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast({ variant: 'destructive', title: "Error", description: "Fallo al subir voucher." }); 
    } finally { 
    ...
    <div className="container mx-auto p-4 max-w-6xl">
      <div className="flex flex-col md:flex-row justify-between items-end gap-4 mb-8">
        ...
        <div className="flex items-center gap-4">
          <Button onClick={handleTestLioren} variant="outline" size="sm" className="bg-blue-50 text-blue-700 font-bold" disabled={isProcessing === "testing"}>
            <FlaskConical className="h-3.5 w-3.5 mr-2" /> TEST CONEXIÓN SII
          </Button>
              ...
              return (
                <TableRow key={quote.id} className="text-xs hover:bg-slate-50 transition-colors">
                  ...
                  <TableCell className="text-center">
                    <Badge className={`font-bold text-[9px] uppercase px-2 py-0.5 rounded-none border-none text-white ${status === 'FACTURADO' ? 'bg-emerald-500' : status === 'PAGADO' ? 'bg-blue-700' : status === 'CORREO_ENVIADO' ? 'bg-indigo-600' : 'bg-amber-500'}`}>
                      {status}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-right px-6">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-56">
                        ...
                        {status === 'PAGADO' && <DropdownMenuItem className="text-xs font-bold text-emerald-600" onClick={() => handleInvoiceNow(quote.id)}><FileText className="mr-2 h-3.5 w-3.5" /> Facturar Individual</DropdownMenuItem>}
                        {pdfUrl && <DropdownMenuItem className="text-xs font-bold text-blue-600" onClick={() => window.open(pdfUrl, '_blank')}><FileText className="mr-2 h-3.5 w-3.5" /> VER FACTURA SII</DropdownMenuItem>}
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </TableCell>
                </TableRow>
                ...
                <div className="flex gap-2">
                  {['CONFIRMADA', 'CORREO_ENVIADO'].includes(mapLegacyStatus(quoteToManage.status)) && (
                    <Button onClick={() => handleSendEmail(quoteToManage)} disabled={isProcessing === quoteToManage.id} className="text-[10px] font-bold h-8 px-4 bg-slate-900">
                      {isProcessing === quoteToManage.id ? <Loader2 className="animate-spin h-3 w-3 mr-2"/> : <Send className="h-3 w-3 mr-2" />} ENVIAR
                    </Button>
                  )}
                  ...
                  {(quoteToManage.liorenPdfUrl || (quoteToManage as any).liorenPdfUrl) && (
                    <Button asChild className="text-[10px] font-bold h-8 px-4 bg-blue-700">
                      <a href={quoteToManage.liorenPdfUrl || (quoteToManage as any).liorenPdfUrl} target="_blank" rel="noopener noreferrer">
                        <FileText className="h-3 w-3 mr-2" /> VER FACTURA SII
                      </a>
                    </Button>
                  )}
                </div>
