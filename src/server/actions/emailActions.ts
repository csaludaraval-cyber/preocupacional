...
export async function enviarConfirmacionPago(quote: any) {
  ...
  // 1. Construir lista dinámica de trabajadores
  ...
  // 3. Personalizar contenido según tipo de cliente
  const config = {
    asunto: esFrecuente 
      ? `📅 CITA CONFIRMADA - Orden #${id.slice(-6).toUpperCase()} - ARAVAL` 
      : `✅ PAGO CONFIRMADO - Orden #${id.slice(-6).toUpperCase()} - ARAVAL`,
    tituloHeader: esFrecuente ? "Cita Médica Confirmada" : "Confirmación de Pago",
    mensajePrincipal: esFrecuente 
      ? `Le informamos que la atención médica ha sido agendada exitosamente bajo su <strong>convenio de facturación mensual</strong> para la empresa <strong>${empresaData?.razonSocial}</strong>.`
      : `Confirmamos la recepción exitosa del pago por los servicios médicos de la empresa <strong>${empresaData?.razonSocial}</strong>.`,
    mostrarMonto: !esFrecuente
  };

  const montoHtml = config.mostrarMonto ? `
    <p style="margin-bottom: 5px;"><strong>Monto Recibido:</strong> ${new Intl.NumberFormat('es-CL', { style: 'currency', currency: 'CLP' }).format(total)}</p>
  ` : '';
    ...
    html: `
      <div style="font-family: sans-serif; color: #334155; line-height: 1.6; max-width: 600px; margin: auto; border: 1px solid #e2e8f0; border-radius: 16px; overflow: hidden; background-color: white;">
        <div style="background-color: #0a0a4d; padding: 30px; text-align: center;">
          <h1 style="color: white; margin: 0; font-size: 20px; text-transform: uppercase; letter-spacing: 1px;">${config.tituloHeader}</h1>
        </div>
        <div style="padding: 30px;">
          <p>Estimado/a <strong>${solicitanteData?.nombre || 'Solicitante'}</strong>,</p>
          <p>${config.mensajePrincipal}</p>
          ...
          <p style="margin-top: 10px; font-size: 13px; color: #475569;">* Cada trabajador debe presentar su Cédula de Identidad y seguir estrictamente las indicaciones de ayuno y preparación enviadas anteriormente.</p>
